...
<style>
/* Dark is the default look regardless of OS preference - the toggle opts
   into light, rather than the usual light-default/dark-opt-in pattern. */
:root {{
...
(function () {{
  ...
  if (btn) {{
    btn.addEventListener('click', function () {{
      var next = isDark(root.getAttribute('data-theme')) ? 'light' : 'dark';
      apply(next);
...
def render(doc: dict, all_docs: dict = None) -> str:
    ...
    return PAGE.format(
        ...
        assets=assets_html, record=track_record_section(all_docs or {doc['date']: doc}),
    )
